...
async function seed() {
  ...
  try {
    ...
    const [existingClient] = await db.select().from(clients).where(eq(clients.company_name, 'PharmaCore Ltd')).limit(1);
    ...
    if (clientForApp) {
        const [existingApp] = await db.select().from(applications).where(eq(applications.product_name, 'Panadol Extra')).limit(1);
        if (!existingApp) {
