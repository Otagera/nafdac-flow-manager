      ...
      <div className="flex justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-4">
            <div className="hidden sm:block text-sm text-muted-foreground bg-slate-100 px-3 py-1 rounded-full border border-slate-200 whitespace-nowrap">
                <span className="hidden lg:inline">Active Session: </span><span className="font-semibold text-slate-900">{user.username} ({user.role})</span>
            </div>
        ...
        <div className="flex flex-col space-y-1">
            <Link 
                ...
                className={`p-2 rounded-md text-sm font-medium transition-all flex items-center gap-3 border border-transparent ${
                    role === 'FINANCE' ? 'text-yellow-700 hover:bg-yellow-50' :
                    role === 'VETTING' ? 'text-blue-700 hover:bg-blue-50' :
                    role === 'DIRECTOR' ? 'text-purple-700 hover:bg-purple-50' :
