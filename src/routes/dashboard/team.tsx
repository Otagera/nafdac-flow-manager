                        ...
                        {users.map((u) => {
                            ...
                            return (
                                <TableRow key={u.id}>
                                    ...
                                    <TableCell>
                                        {u.invite_code ? (
                                            <span className="text-yellow-600 bg-yellow-100 px-2 py-1 rounded text-xs">Invited</span>
                                        ) : (
...
function InviteUserDialog({ onInvite }: { onInvite: () => void }) {
    ...
    const handleInvite = async () => {
        const { data, error } = await api.admin.users.invite.post({ username, role })
        if (error) {
                ...
                {!generatedCode ? (
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="e.g. finance_lead" />
                        </div>
                        <div className="space-y-2">
                            <Label>Role</Label>
                        ...
                        <Button className="w-full" onClick={handleInvite} disabled={!username}>Generate Code</Button>
                    </div>
                ) : (
                    <div className="space-y-4 py-4">
