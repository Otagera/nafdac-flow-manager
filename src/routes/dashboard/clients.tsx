...
function ClientManagement() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this company? This might fail if they have active applications.')) return;
...
function AddClientDialog({ onAdd }: { onAdd: () => void }) {
    ...
    const handleAdd = async () => {
        setLoading(true)
        const { data, error } = await api.clients.index.post({ company_name: name, cac_number: cac })
        setLoading(false)
            ...
            <DialogContent>
                ...
                <div className="space-y-4 py-4">
                    <div className="space-y-2">
                        ...
                        <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. HealthPlus Ltd" />
                    </div>
                    <div className="space-y-2">
